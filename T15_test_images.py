...
def test_blue(original,new)->None:
    ...
    Function checks each pixel of the copies image with the applied  blue filtered 
    image and tests if each pixel only has the blue component and returns if the filter
    was successful
...
def test_three_tone() -> None:
    ...
    """Author: Joahkim Vaudrin-Moisan 101153491
    Function creates a 3x1 image and tests if the pixels have the correct colour 
    configuration, being the first colour if the average RGB value is lower than 
    85, the middle colour if the average is between 85 and 170, and the latter if
...
def test_extreme()->None:
    ...
    """Author: Dorothy Tran 101141902
    Function creates a 3x1 image created with the applied extreme contrast filter and 
...
def test_sepia() -> None:
    ...
    '''Author:Kian Zalzalah 101148652
    Function creates a 3x1 image and tests the detect edges filter by passing a 
    new image through the filter  with expected values to ensure properties of 
    filter are correct.
    >>> test_sepia()
    Checking pixel @(0, 0) PASSED
    ------
...
def test_posterize()->None:
    ...
    """Author: Dorothy Tran 101141902
    Function creates a 3x1 image created with the applied posterized filter and 
    tests if the first three pixels are filtered with the expected rgb values
    >>> test_posterize()   
    Checking pixel @(0, 0) PASSED
    ------
    Checking pixel @(1, 0) PASSED
...
def test_detect_edges() -> None:
    ...
    """Author: Kian Zalzalah 101148652
    Function creates a 2x2 image and tests the detect edges filter by passing a 
    new image through the filter with expected values to ensure properties of 
    filter are correct.
    >>>test_detect_edges()
    Checking pixel @(0, 0) PASSED
...
def test_detect_edges_better() -> None:
    ...
    """Author: Kian Zalzalah 101148652
    Function creates a 2x2 image and tests the improved detect edges filter by 
    passing a new image through the filter with expected values to ensure properties 
    of filter are correct.
...
def test_flip_vertical()-> None: 
    ...
    """Author: Dorothy Tran 101141902  
    Function creates a 3x1 image and tests to see if each pixel of the image is
...
def test_flip_horizontal()->None:
    ...
    """Author: Dorothy Tran 101141902 
    Function creates a 3x1 image and tests to see if each pixel of the image is 
    horizontally respect to the y axis flipped comparing to the original image
